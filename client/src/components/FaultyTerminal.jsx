...
function onTouchStart(e) {
  if (e.touches.length > 0) {
    ...
    for (const [elem, t] of map) {
      ...
      if (inside(rect)) {
        t.touching = true; updateNorm(t, rect);
...
function updateNorm(t, rect) {
  const { position: i, nPosition: s } = t;
  i.x = pointer.x - rect.left; i.y = pointer.y - rect.top;
  s.x = (i.x / rect.width) * 2 - 1; s.y = (-i.y / rect.height) * 2 + 1;
}
  ...
  update(e) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      if (idx === 0 && this.config.followCursor === false) {
        ...
        TMP.scale.setScalar(this.physics.sizeData[idx]);
      }
      TMP.updateMatrix();
